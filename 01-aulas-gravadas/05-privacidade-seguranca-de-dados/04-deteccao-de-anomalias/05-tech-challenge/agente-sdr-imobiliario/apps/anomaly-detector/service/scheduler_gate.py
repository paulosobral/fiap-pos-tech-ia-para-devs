...
class SchedulingGate:
    ...
    materializada no item da anomalia (`scheduling_restricted=True`,
    `status="open"`) e consultável por GSI `lead-index`. O consumidor real é
    o checker da U1 (`DynamoRestrictionCheck` em
    apps/conversation-router/service/restriction.py), que o fluxo de
    agendamento consulta antes de confirmar visitas de leads sinalizados.
    """
    ...
    def restrict(self, lead_id: str, anomaly_id: str) -> None:
        ...
        log_event("scheduling_restricted", lead_id=lead_id, anomaly_id=anomaly_id)
