...
class SilenceWindow:
    """FR8.3 — janela de silêncio configurável (horas UTC, fim exclusivo).

    Fora da janela nenhum follow-up é enviado: o tick marca os leads como
    `deferred` e eles permanecem pendentes para o próximo tick dentro da
    janela. Suporta janela que cruza a meia-noite (start > end) e janela 24h
    ...
    def from_env(cls, environ: dict[str, str] | None = None, now_fn: Callable[[], datetime] = utc_now) -> "SilenceWindow":
        ...
        return cls(start_hour=start, end_hour=end, now_fn=now_fn)

    def is_open(self, now: datetime | None = None) -> bool:
        hour = (now or self.now_fn()).hour
