...
class ConversationStore:
    """Leitor do Contrato 5 (sessões) — U7 é reader, U1 é owner.

    Duas varreduras paginadas sobre a tabela compartilhada: `CONV#` devolve as
    conversas (created_at, current_state, context) e `PROFILE` o mapa
    `lead_id → perfil` (status, intent, area, route). Itens sem
    identificadores são ignorados com log (nunca quebram a agregação).
    DynamoDB throttling/erros viram `ConversationStoreError` (Lambda → 500).
    """
    ...
    def get_lead_profiles(self) -> dict[str, dict[str, Any]]:
        ...
        for item in self._scan_by_prefix(PROFILE_SK):
            ...
            if not lead_id:
                log_event("profile_item_missing_lead_id", level=logging.WARNING)
                continue
    ...
    def _scan_by_prefix(self, prefix: str) -> list[dict[str, Any]]:
        ...
        try:
            while True:
                ...
                for raw in response.get("Items", []):
                    ...
                    if not item.get("session_id") or not item.get("lead_id"):
                        log_event("conversation_item_missing_identifiers", level=logging.WARNING)
                        continue
                    items.append(item)
            ...
            log_event("session_store_unavailable", level=logging.ERROR, error=str(exc))
            raise ConversationStoreError("session store unavailable") from exc
        return items
